...
使用一个中介对象来封装一系列的对象交互。中介者使各个对象不需要显式地相互引用， 从而使其耦合松散，而且可以独立地改变它们之间的交互。（通过类之间互相发送消息实现类之间的协调工作。）例如 MVC 架构中，Controller 充当着 Model 和 View 的中介者，在前后端交互时起到了中间人的作用。(类似外观模式？)
...
中介者模式中，Mediator 类承担了核心责任，如果设计不当，会导致 Mediator 变得过于复杂，难以维护。
...
2. Memento: 备忘录对象，负责保存 Originator 的内部状态。
3. Caretaker: 守护者对象，负责保存，读取备忘录对象。

备忘录模式给用户提供了一种可以恢复状态的机制，让用户能够比较方便地回到某个历史阶段的状态。而且它实现了信息封装，用户无需关心状态的保存细节。

## 解释器模式

`design/interpreter`

在编译原理中，一个表达式通过词法分析器形成词法单元，而后这些词法单元再通过语法分析器构建语法分析树，最终形成一颗抽象的语法分析树。这里的词法分析器和语法分析器都可以看作是解释器。

解释器模式的思想即来源于编译原理。是指给定一个表达式，定义它的文法的一种表示，并定义一个解释器，使用该解释器来解释语言中的句子（表达式）。

使用解释器模式设计解决加减运算的问题，如输入一个表达式 `a+b-c` 然后再分别输入 a, b, c 的值，返回这个表达式的结果。

## 状态模式

`design/state`

状态模式主要用来解决对象再多状态转换时，需要对外输出不同的行为的问题。状态和行为是一一对应，状态之间可以相互转换。

状态模式中涉及的角色及职责

1. State 类 抽象状态角色，定义一个接口封装与 Context 的一个特点接口相关行为。
2. ConcreteState 具体的状态角色，每个子类实现一个与 Context 的一个状态相关行为。
3. Context 类，用于维护 State 实例，这个实例定义当前状态。

## 策略模式

`design/strategy`

类继承中的问题。子类对父类的局部改动会影响其他部分，甚至可能子类需要重写父类的所有方法。这样会带来类的溢出效应。这就需要策略模式来解决，即将继承关系改成组合或聚合关系。

策略模式，定义算法族并分别封装起来，让他们之间可以互相替换。此模式让算法的变化独立于使用算法的客户。这算法体现了几个设计原则：1. 把变化的代码从不变的代码中分离出来；2.针对接口编程而不是类;3.使用组合/聚合，而不是继承。

## 职责链模式

`design/responsibility`

采购问题：如果采购金额<=5000，由部门副经理审批；如果采购金额>5000,并且<=10000，由部门经理审批；如果金额>10000 且<=50000 由副总审批；如果金额大于 50000，又总经理审批。

如果不采用设计模式解决该问题，则应该是收到一个采购请求后，根据金额来调用对应的审批角色，这样就需要 if-else 判断。如果各个级别人员的审批金额发生变化，维护起来就很麻烦，而且主程序需要知道有多少个审批级别。基于此，我们需要采用职责链模式来处理。

责任链模式为请求创建一个接收者对象的链条。每个接收者对象都包含对另一个接收者对象的引用，如果这个对象不能或无权处理该请求，那么它就把请求传给下一个接收者，知道找到正确的接收者为止。
